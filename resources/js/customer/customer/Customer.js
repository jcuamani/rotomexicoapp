$(document).ready(function()
{
    ...
    var table = $('#dt_Customer').DataTable({            
        ...
        "language": {
            ...
            ajax: window.appRoutes.Customer_Customer_Customer_Index,
            buttons: [
                {
                    extend: 'excelHtml5',
                    title: 'Listado_Roles',
            ...
            columns: [                        
                    ...
                    },       
                    {
                        ...
                        render: function (data, type) {                            
                            return type === 'display' && data == 1 ?
                            '<span class="d-inline-flex align-items-center p-1 pe-2 rounded-1 text-white bg-success fs-10"><i class="ti ti-point-filled me-1 fs-11"></i>'+window.appTranslates.Aprobado+ '</span>':
                            '<span class="d-inline-flex align-items-center p-1 pe-2 rounded-1 text-white bg-danger fs-10"><i class="ti ti-point-filled me-1 fs-11"></i>'+window.appTranslates.NoAprobado+ '</span>';
                        },                        
                    },       
                    {
                        data: 'acoountid', 
                    ...
                    {
                        data: 'last_web_order_at', 
                        name: 'last_web_order_at',
...
if($('#idshopaccounttype').length > 0) 
{
    $("#representation_behaviour").hide(200); 
    $('#idshopaccounttype').on('select2:select', function (e) {
        var data = e.params.data;            
        opc = data.text;
        var arrayids= opc.split(" - ");
        opcsel = $.trim(arrayids[0]);
        //alert("|"+opcsel+"|");
        if(opcsel =='001' ) {

            $("#representation_behaviour").hide(200);     
            $("#account_role").show(200);         

        } else if(opcsel =='002' ) {

            $("#account_role").hide(200);       
            $("#representation_behaviour").hide(300);       

        } else if(opcsel =='003' ) {
            
            $("#account_role").hide(200);       
            $("#representation_behaviour").show(200);       

        }
        
    }); 
    
    $("#idshopaccounttype").trigger('change.select2');        

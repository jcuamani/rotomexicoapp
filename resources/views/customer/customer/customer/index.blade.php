        ...
        <div class="page-header">
            <div class="add-item d-flex">
                <div class="page-title">
                    <h4 class="fw-bold">{{__("application_lang.application_clients")}}</h4>
                    <h6>{{__("application_lang.app_menus_lbl_btn_actions_manage")}} {{__("application_lang.app_lbl_yours")}} {{__("application_lang.application_clients")}}</h6>
                </div>
            </div>
            ...
            <div class="page-btn">
                @can('customer_customer_customer.create')
                    <a href="{{ secure_route('customer.customer.customer.addEdit',['id' => encrypt_param(0)]) }}" class="btn btn-primary ">
                        <i class="ti ti-circle-plus me-1"></i>
                        {{__("application_lang.application_client_add_new")}}
                    </a>
        ...
        <div class="card">
            <div class="toast-container position-fixed top-0 end-0 p-3">
                @if(count($errors) > 0)
                    ...
                    <div id="dangerToast" class="toast colored-toast bg-danger" role="alert"
                ...
                @if (Session::has('success'))
                    ...
                    <div id="successToast" class="toast colored-toast bg-success" role="alert"
                        aria-live="assertive" aria-atomic="true">
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table datatable" id="dt_Customer">
                        <thead class="thead-light">
                            <tr>                                
                                <th>{{__("application_lang.app_menus_lbl_tb_col_name")}} </th>
                                <th>{{__("application_lang.application_email")}} </th>
                                <th>{{__("application_lang.app_lbl_status")}}</th>                                    
                                <th>{{__("application_lang.app_lbl_approved")}}</th>                                    
                                <th>{{__("application_lang.app_lbl_account_type")}}</th>                                    
                                <th>{{__("application_lang.app_lbl_account_id")}}</th>                                    
                                <th>{{__("application_lang.app_lbl_last_login")}}</th>                                    
                                <th>{{__("application_lang.app_lbl_last_web_order")}}</th>                                    
